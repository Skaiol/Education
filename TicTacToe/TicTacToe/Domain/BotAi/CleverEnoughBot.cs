...
{
    public sealed class CleverEnoughBot : BotProcessor
    {
        private readonly List<MoveLocation> _all;
        private readonly Random _random;
        private readonly List<List<MoveLocation>> _winConditions;

        public CleverEnoughBot(TicTacToeGame game, List<MoveInfo> moves) : base(game, moves)
        {
            _random = new Random();
        ...
        protected override MoveLocation CalcNextLocation()
        {
            ...
            {
                return Moves.Select(x => x.Location).Contains(MoveLocation.Center) ? RandomAngle() : MoveLocation.Center;
            }

            if (Game.MovesCount == 2)
            {
                List<MoveLocation> madeMoves = Moves.ConvertAll(x => x.Location);
                var botMoves = Moves.Where(x => x.Player.Id == Game.PlayerWhoMoves.Id)
                    .Select(x => x.Location)
                    .ToList();
                var condition = _winConditions.First(x => x.Except(botMoves).Count() == 2 && x.Except(madeMoves).Count() == 2);
                return condition.Except(botMoves).First();
            }

            return WinOrNotLooseMove();
        }
        ...
        private MoveLocation RandomAngle()
        {
            ...
            var index = _random.Next(angleMoves.Count);
            return angleMoves[index];
        }

        private MoveLocation WinOrNotLooseMove()
        {
            List<MoveLocation> madeMoves = Moves.ConvertAll(x => x.Location);
